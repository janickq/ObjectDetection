...
#                Filename            BGR           text
templateInfoList = [ ["Images3/Red2.png",  (0, 0, 255),  "Red"],
                 ["Images3/Blue2.png", (255, 0, 0), "Blue"],
                 ["Images3/Yellow2.png", (0, 255, 255), "Yellow"],
                 ["Images3/Gurney2.png", (0, 0, 0), "Gurney"] ]
...
ap.add_argument("-b", "--threshold", type=float, default=0.75,
...
for i in range(numOfTemplate):
    ...
    #template[i] = cv2.cvtColor(template[i], cv2.COLOR_BGR2HSV)
...
while True:
    ...
    frame_got, imageColor = cam.read()
    if frame_got is False:
        break
        
    image = imageColor #cv2.cvtColor(imageColor, cv2.COLOR_BGR2HSV)    
    ...
    for i in range(numOfTemplate):
        ...
        for (startX, startY, endX, endY) in pick:
	        # draw the bounding box on the image
	        cv2.rectangle(imageColor, (startX, startY), (endX, endY),
		        templateColor[i], 3)

    # show our output image *before* applying non-maxima suppression
    cv2.imshow("Before NMS", clone)
    # show the output image
    cv2.imshow("After NMS", imageColor)
